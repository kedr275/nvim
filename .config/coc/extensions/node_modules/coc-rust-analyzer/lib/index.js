...
var viewSyntaxTree = new lc.RequestType(
...
function createClient(bin, config) {
  ...
  const initializationOptions = import_coc.workspace.getConfiguration("rust-analyzer");
...
var Config = class {
  ...
  get disablePullDiagnostic() {
    return this.cfg.get("disablePullDiagnostic");
  }
};
...
`;
  ...
  await import_node_fs.promises.rename(dest, origFile);
  ...
  await import_node_fs.promises.unlink(origFile);
}
function isMusl() {
...
async function downloadServer(context, release) {
  ...
  const _path = import_node_path.default.join(context.storagePath, release.name);
  const randomHex = (0, import_node_crypto.randomBytes)(5).toString("hex");
  const tempFile = import_node_path.default.join(context.storagePath, `${release.name}${randomHex}`);
  if (process.platform === "win32") {
    await import_node_fs.promises.writeFile(tempFile, await resp.buffer());
    new import_adm_zip.default(tempFile).extractAllTo(context.storagePath, true, true);
    await import_node_fs.promises.unlink(tempFile).catch((err) => {
      console.error(err);
    });
    statusItem.hide();
    return;
  }
  const destFileStream = (0, import_node_fs.createWriteStream)(tempFile, { mode: 493 });
  ...
  await import_node_fs.promises.unlink(_path).catch((err) => {
    if (err.code !== "ENOENT") throw err;
  });
  await import_node_fs.promises.rename(tempFile, _path);
  await context.globalState.update("release", release.tag);
  try {
    if (await import_node_fs.promises.stat("/etc/nixos")) {
...
var Ctx = class {
  ...
  resolveBin() {
    ...
    let bin = (0, import_node_path2.join)(this.extCtx.storagePath, executableName);
...
function viewSyntaxTree2(ctx) {
  return async () => {
    ...
    const ret = await ctx.client.sendRequest(viewSyntaxTree, param);
    if (!ret) return;
...
function openDocs2(ctx) {
  return async () => {
    ...
    if (doclink) {
      if (doclink.local) {
        const exist = (0, import_node_fs3.existsSync)(import_coc5.Uri.parse(doclink.local).fsPath);
        if (exist) {
...
`;
    const tempFile = (0, import_node_path3.join)((0, import_node_os.tmpdir)(), `${(0, import_node_crypto2.randomBytes)(5).toString("hex")}.html`);
    (0, import_node_fs3.writeFileSync)(tempFile, html, { encoding: "utf-8" });
...
async function activate(context) {
  ...
  if (!(0, import_node_fs4.existsSync)(serverRoot)) {
    (0, import_node_fs4.mkdirSync)(serverRoot);
  }
  const bin = ctx.resolveBin();
  if (!bin) {
  ...
  ctx.registerCommand("viewSyntaxTree", viewSyntaxTree2);
